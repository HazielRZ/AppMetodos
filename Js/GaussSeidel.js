...
// Lógica de Gauss-Seidel
        let X = new Array(n).fill(0); // Vector de soluciones
        let error = 100;
        let iter = 0;
        const iterationsData = [];

        while (error > tolerance && iter < maxIter) {
            let maxErrorInIteration = 0;
            let X_old = [...X]; // Guardamos el estado anterior para calcular el error
            ...
            for (let i = 0; i < n; i++) {
                ...
                if (currentError > maxErrorInIteration) {
                    maxErrorInIteration = currentError;
                }
            }

            error = maxErrorInIteration;
            iter++;
